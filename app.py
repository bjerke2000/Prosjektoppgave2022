...
class ItemModel(db.Model):
    ...
    group_privs = db.Column(
        db.PickleType
    )  # lagra privs i dictionaries ved å bruk pickle (var = pickle.dumps(innhold) / pickle.loads(var)) Pickle Rick :D
    tags = db.Column(
        db.String(500)
    )  # py list with id of tags where [] are replaced with ',' ",0,1,60,89,"
    private = 0
    editable = 0
    ownername = ""
    filetype = ""
...
class CommentsModel(db.Model):
    ...
    )  # fk to Items id, that has the comment section.
    user_id = db.Column(
        db.Integer, db.ForeignKey("users.id")
    )  # fk to users id, to know who made the comment
...
def PermissionHandler(
    required_priv, object
):  # Checks if the current user has the given priv for an item
    ...
    for group, priv in group_privs.items():
        if (str(group) in user_groups and required_priv in priv) or str(
            ADMINGROUP
        ) in user_groups:
            return True
    return False
...
def PermissionCreator(
    form, group_priv_dict={}
):  # Makes priv dictionairy from input lists
...
def GetAvaliableName(
    path, name, iteration
):  # Iterates through all folders with the same name in a list, until it gets an avaliable name
    item = ItemModel.query.filter_by(
        path=path, itemname=name + "(" + str(iteration) + ")"
    ).first()
    if item is None:
        return iteration
    else:
...
def TagManager(
    tags,
):  # if tag already exists: append id to list, else: create tag and retrieve recieved id and append to list
    tags = tags.split(",")
    cleaned_tags = []
    final_tag_str = ","
    for tag in tags:
        temptag = ""
        cleaned_tags.append(
            temptag.join(letter for letter in tag if letter.isalnum()).lower()
        )  # Remove all special charactes and set them to lower case
    ...
    db.session.commit()
    newtags = TagModel.query.filter(
        TagModel.tag.in_(cleaned_tags)
    ).all()  # get new tag objects
    for tag in newtags:
...
def DeleteUser(
    id,
):  # Deletes a user and reassigns all items and comments from the user to [DELETED USER]
...
def admin():
    if str(ADMINGROUP) in current_user.groups.split(","):
        ...
        for comment in Comments:
            comment.username = (
                UserModel.query.filter_by(id=comment.user_id).first().name
            )
...
def item(path, name):
    ...
    match item.type:
        case 0:  # Show contents of folder
            unchecked_contents = ItemModel.query.filter_by(
                path=f"{item.path}{item.itemname.strip('-').split('~')[0]}-"
            )
            contents = []
            for item in unchecked_contents:
                if PermissionHandler("r", item):
                    if ALLUSERSGROUP in item.group_privs:
                        ...
                        item.private = 1
                    if item.type == 1:
                        type = item.itemname.split("~")[1].split(".")[
                            -1
                        ]  # Gets filetype
        ...
        case 1:  # Show contents if file
            ...
            item.ownername = (
                UserModel.query.filter_by(id=item.owner).first().name
            )  # gets username for displaying
            if type in text_types:
                with open(
                    os.path.join(app.config["UPLOAD_FOLDER"], item.itemname),
                    "r",
                    encoding="utf-8",
                ) as f:
                    lines = f.readlines()
...
def newfolder(path, parent):
    ...
    if form.validate_on_submit():
        ...
        item = ItemModel.query.filter_by(
            path=itempath, itemname=form.itemname.data
        ).first()
        if item is None:
            foldername = form.itemname.data
        else:
            foldername = GetAvaliableName_helper(
                itempath, form.itemname.data
...
def addfile(path, parent):
    ...
    if form.validate_on_submit():
        ...
        if form.name.data != "":
            itemname = secure_filename(
                form.name.data + "." + form.file.data.filename.split(".")[-1]
            )
        else:
